...
function signEnvelope(action, payload, secret, timestamp, nonce) {
  return crypto
    ...
    .update(`${action}.${timestamp}.${nonce}.${JSON.stringify(payload)}`)
    .digest("hex");
}
...
async function callAppsScript(action, payload) {
  ...
  const signature = signEnvelope(action, payload, secret, timestamp, nonce);
  const response = await fetch(url, {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: JSON.stringify({ action, payload, timestamp, nonce, signature }),
